...
def test_optimize_endpoint_validation_errors(api_client):
    ...
    # 3. Same locations
    r = api_client.post(url, {"start": "Chicago, IL", "destination": "Chicago, IL"}, format="json")
    assert r.status_code == status.HTTP_400_BAD_REQUEST
...
@patch("requests.post")
def test_optimize_endpoint_success(mock_post, mock_geocode, api_client, mock_fuel_stations):
    url = reverse("optimize_route")
    ...
    assert r.data["total_gallons"] == 0.0  # distance is 180 miles < 500 range, no fuel stop needed
    assert len(r.data["fuel_stops"]) == 0
    assert "route_geometry" in r.data
    
    # Verify Cache was created
    assert RouteCache.objects.filter(start_query="COLUMBUS, OH", destination_query="INDIANAPOLIS, IN").exists()
